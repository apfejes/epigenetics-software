...
class AbstractRpcServer(object):
    ...
    def Send(self, request_path, payload = None,
        ...
        """
        # TO DO: Don't require authentication.    Let the server say
...
def RunShellWithReturnCode(command, print_output = False,
    ...
    out, _err, retcode = RunShellWithReturnCodeAndStderr(command, print_output,
    ...
    def UploadBaseFiles(self, issue, rpc_server, patch_list, patchset, options,
        ...
        def UploadFile(filename, file_id, content, is_binary, status, is_base):
            ...
            if options.verbose > 0 and not file_too_large:
                print "Uploading %s file for %s" % (itype, filename)
...
class SubversionVCS(VersionControlSystem):
    ...
    def _GuessBase(self, required):
        ...
        if url:
            ...
            guess = ""
            # TO DO(anatoli) - repository specific hacks should be handled by server
...
class CVSVCS(VersionControlSystem):
    ...
    def GetOriginalContent_(self, filename):
        ...
        # TO DO need detect file content encoding
...
class PerforceVCS(VersionControlSystem):
    ...
    def GetFileProperties(self, property_key_prefix = "",):
        description = self.RunPerforceCommand(["describe", self.p4_changelist],
                                                                                    marshal_output = True)
    ...
    def IsBaseBinary(self, filename):
        ...
        return self.IsBinaryHelper(base_filename)

    def IsPendingBinary(self, filename):
        return self.IsBinaryHelper(filename)

    def IsBinaryHelper(self, filename):
        file_types = self.GetFileProperties("type")
...
def SplitPatch(data):
    ...
    for line in data.splitlines(True):
        new_filename = None
        if line.startswith('Index:'):
            _unused, new_filename = line.split(':', 1)
            new_filename = new_filename.strip()
        elif line.startswith('Property changes on:'):
            _unused, temp_filename = line.split(':', 1)
...
def GuessVCSName(options):
    ...
    def RunDetectCommand(vcs_type, command):
        ...
        except OSError, (errcode, _message):
...
def RealMain(argv, data = None):
    ...
    base_hashes = ""
    for fi, info in files.iteritems():
